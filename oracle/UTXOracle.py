...
def Ask_Node(command):
    # Establish connection to the Bitcoin Core node
    try:
        proxy = RawProxy(service_url=f'https://nameless-wild-glade.btc.quiknode.pro/')

        print(command)
        # Call the RPC method using the provided command
        result = proxy._call(command[0], *command[1:])
        ...
        print("\t1) Make sure Bitcoin Core is running and reachable.")
        print("\t2) Check your RPC username and password.")
        print("\t3) Make sure config file bitcoin.conf has server=1")
        print("\nThe command was:", command)
        print("\nThe error from Bitcoin Core was:\n")
        print(e)
        exit()
...
def Run_Oracle(date):
    ...
    try:
        ...
        if date.timestamp() >= latest_utc_midnight.timestamp():
        ...
        if date.timestamp() < july_26_2020.timestamp():
    ...
    price_day_seconds = int(date.timestamp())
    price_day_date_utc = date.strftime("%B %d, %Y")
